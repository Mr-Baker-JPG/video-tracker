...
import { VelocityVsTimeGraph } from './velocity-vs-time-graph.tsx'
...
test('Graph component shows empty state when no tracking data', () => {
	render(<VelocityVsTimeGraph trackingPoints={[]} scale={null} axis={null} />)
...
test('Velocity calculation is correct for sample data', () => {
	// Test with known data: object moves 50 pixels in 1 second (30 frames)
	// Velocity should be 50 pixels/s
	const trackingPoints = [
		{ frame: 0, x: 100, y: 200, trackingObjectId: 'obj1' },
...
test('Velocity handles edge cases (first/last frames)', () => {
	// Test with single point (should have velocity 0)
	...
	const { unmount: unmount2 } = render(
		<VelocityVsTimeGraph trackingPoints={twoPoints} scale={null} axis={null} />,
	)
	expect(screen.getByRole('tab', { name: /x axis/i })).toBeInTheDocument()
	unmount2()
